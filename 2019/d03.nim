...
type
  ...
  pointList = seq[point]
  instruction = tuple[dir:char, dist:int]

proc intersectionList(x,y: pointList): pointList =
  let
    a = toHashSet x
    b = toHashSet y
    c = a.intersection(b)
...
# use 39ms to compute using hashset
proc shortestDistance(pl: pointList):int =
  var distant = high(int)
  for p in pl:
    let d = abs(p.x) + abs(p.y)
    if d < distant: distant = d
  return distant

# use 42.6 second to compute input
proc shortestDistanceA(x,y: pointList):int =
  var distant = high(int)
  for p in x:
    if p in y:
      let d = abs(p.x) + abs(p.y)
...
proc tracePoint(wire:string): pointList =
  var (x,y) = (0,0)
  let turns = wire.split(',')

  for turn in turns:
    let t:instruction = (turn[0], turn[1..^1].parseInt)
    case t.dir
      of 'U':
        for i in 1..t.dist:
          result.add (x, y+i)
      ...
      of 'R':
        ...
        raise newException(Exception, "not valid direction")

proc stepCount(pl:pointList, p:point): int =
  result = 0
  for i in pl:
    result += 1
    if i == p:
